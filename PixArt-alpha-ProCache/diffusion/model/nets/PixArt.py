...
class PixArtBlock(nn.Module):
    ...
    1. Full Fresh (is_force_fresh=True): Compute all, update cache.
    2. Partial Fresh (is_force_fresh=False, cache_only=False): Compute changed tokens (Cross/MLP), update cache.
    3. Cache Only (is_force_fresh=False, cache_only=True): Read all from cache, no computation.
    """
    ...
    def forward(self, x, y, t, current, cache_dic, mask=None, **kwargs):
        ...
        cal_type = global_force_fresh(cache_dic, current)
        current['type'] = cal_type
